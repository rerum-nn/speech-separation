...
class RTFSNet(nn.Module):
    def __init__(
        ...
        assert hidden_dim % (2 ** (compression_blocks - 1)) == 0, "hidden_dim must be divisible by 2 ** compression_blocks"
        ...
        self.s3_mask_generator = S3MaskGenerator(cur_dim, n_spks=n_spks, one_pass=one_pass)
